...
(define (get-pref-list pref person)
  (define rezultat
   (filter (lambda (x) (equal? person (car x))) pref)
  )
  (if (null? rezultat) '() (cdar rezultat))
  )
...
(define (preferable? pref-list x y)
  (if (equal? x y) #f
      (if (equal? (member y (member x pref-list)) #f) #f #t)))
...
(define (change-first p L val)
  (define rezultat 
    (find-first p L))
  (if (not (equal? rezultat #f)) (map (lambda (x) (if (equal? x rezultat) val x)) L)
           L)
  )
...
;;;;aici a trebuit sa schimb functia pentru ca nu o facusem prea corect in etapa1
...
; Precizări (a5specte care se garantează, nu trebuie verificate):
; - fiecare cuplu din lista engagements are pe prima poziție
;   o femeie

(define (stable-match? engagements mpref wpref)
  (foldr (lambda (x acc) (and acc (not (better-match-exists? (cdr x) (car x) (get-pref-list mpref (cdr x) ) wpref engagements)))) '() engagements)
)
