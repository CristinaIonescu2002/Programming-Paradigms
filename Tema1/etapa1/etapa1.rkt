...
(define (get-women wpref)
  (get-woman-helper '() wpref))

(define (get-woman-helper list-woman wpref)
  (if (equal? wpref '())
      (append list-woman '())
      (get-woman-helper (append list-woman (list (car (car wpref)))) (cdr wpref))))
...
(define (get-pref-list pref person)
  (cond
    ((equal? person (car (car pref))) (cdar pref))
...
(define (preferable? pref-list x y)
  (and (not (null? pref-list))
       (or (and (equal? (car pref-list) x)
                (member y (cdr pref-list))
                (equal? (car (member y (cdr pref-list))) y))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (cond
    ((null? p1-list) #f)
    ((and (equal? p1 (car (get-pref-list pref2 p2))) (equal? p2 (car p1-list))) #f)
