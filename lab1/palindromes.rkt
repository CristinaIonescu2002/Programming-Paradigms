...
(define (palindrome-list? L)
  (cond
    ((equal? L '()) #t)
    ((equal? (car L) (rev (car L))) (palindrome-list? (cdr L)))
    (else #f)
  ))
...
(define (all-palindromes? n Bases)
  (cond
    ((equal? Bases '()) #t)
    ((and (palindrome? (num->base n (car Bases))) #t) (all-palindromes? n (cdr Bases)))
    (else #f)
  ))
...
(define (b-base n baza nr_pal_in_baza)
  (cond
    ((= baza 11) nr_pal_in_baza)
    ((all-palindromes? n (list baza)) (b-base n (add1 baza) (add1 nr_pal_in_baza)))
    (else (b-base n (add1 baza) nr_pal_in_baza))
  ))
  
(define (first-b-pal start b)
  (if (>= (b-base start 2 0) b) start
...
(define (find L lungime rez)
  (cond
    ((< lungime (length rez)) rez)
    ((and (palindrome? (take L lungime)) (> lungime (length rez))) (find (cdr L) (length (cdr L)) (take L lungime)))
    (else (find (cdr L) (length (cdr L)) (find L (sub1 lungime) rez)))
    ))

(define (longest-palindrome n)
  (list->num (find (num->base n 10) (length (num->base n 10)) '() ))
  )
