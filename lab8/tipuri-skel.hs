...
dotV (V vx1 vy1 vz1) (V vx2 vy2 vz2) = vx1*vx2 + vy1*vy2 + vz1*vx2
...
insertElem BSTNil el = BSTnod el BSTNil BSTNil
insertElem root@(BSTnod value left right) el
  | value == el = root
  | value < el = BSTnod value left (insertElem right el)
  | value > el = BSTnod value (insertElem left el) right

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (BSTnod value left right) el
  | value == el = Just value
  | value < el = findElem right el
  | value > el = findElem left el

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTnod el left right)= inorder left ++ [el] ++ inorder right 

size :: BST a -> Int
size BSTNil = 0
size (BSTnod el left right) = 1 + size left + size right

height :: BST a -> Int
height BSTNil = 0
height (BSTnod el left right) = 1 + max (height left) (height right)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced (BSTnod el left right)
